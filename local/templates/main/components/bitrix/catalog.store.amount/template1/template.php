		...
		<?foreach($arResult["STORES"] as $pid => $arProperty):?>
			<li style="display: <? echo ($arParams['SHOW_EMPTY_STORE'] == 'N' && isset($arProperty['REAL_AMOUNT']) && $arProperty['REAL_AMOUNT'] <= 0 ? 'none' : ''); ?>;">
				<?if (isset($arProperty["TITLE"])):?>
					<a style="color: #006BCE; font-size: 13px; font-family: Montserrat; font-weight: 600" href="<?=$arProperty["URL"]?>"> <?=$arProperty["TITLE"]?></a><br />
				<?endif;?>
				...
				<?else:?>
					<?=GetMessage('S_AMOUNT')?>
				<?endif;?>
				<span style="color: #49C111; font-size: 13px; font-family: Montserrat; font-weight: 600" class="balance" id="<?=$arResult['JS']['ID']?>_<?=$arProperty['ID']?>"><?=$arProperty["AMOUNT"]?></span><br />
				<?
