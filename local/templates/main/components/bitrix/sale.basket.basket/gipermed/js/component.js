;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		init: function(parameters)
		{
			...
			this.nodeBasketClear = document.querySelector('.cart-clear-btn');
		...
		showItemsCountMy: function()
		{
			let itemCountNode=document.querySelector('.cart-head .cart-count');
			let itemTotalCountNode=document.querySelector('.cart-sidebar-order-info-title span');
			if (BX.type.isDomNode(itemTotalCountNode))
			{
				itemTotalCountNode.innerHTML = this.result.BASKET_ITEMS_COUNT;
				itemCountNode.style.display = '';
			}
			if (BX.type.isDomNode(itemCountNode))
			{
				itemCountNode.innerHTML = this.result.BASKET_ITEMS_COUNT + ' ' + this.getGoodsMessage(this.result.BASKET_ITEMS_COUNT);
				itemCountNode.style.display = '';
			}
		},
		...
		bindTotalEvents: function(node)
		{
			...
			BX.bind(this.getEntity(node, 'basket_coupon-btn'), 'click', BX.proxy(this.addCouponAction, this));
		...
		//let target = this.getEntity(document, 'basket-coupon-input');
			console.log(target);
		...
		createBasketItem: function(itemId)
		{
			...
			if (basketItemTemplate)
			{
				...
				if (this.shownItems.length && sortIndex >= 0)
				{
					...
					{
						// insert after
						BX(this.ids.item + this.shownItems[this.shownItems.length -
						1]).insertAdjacentHTML('afterend', basketItemHtml);
		...
		bindActionEvents: function(node, data)
		{
			...
			var entity;
			let nodeBasketClear = this.getEntity(document.querySelector('.cart-body'),'basket-clear');
			BX.bind(nodeBasketClear, 'click', BX.proxy(this.deleteAllAction, this));
			this.checkIsEmptyTimer();
		...
		{
			if(Object.keys(this.shownItems).length===0)
			{
			...
			{
				this.timerId=setTimeout(BX.proxy(this.checkIsEmpty, this), 300);
			}
		},

		checkIsEmptyTimer: function()
		{
			this.timerId=setTimeout(BX.proxy(this.checkIsEmpty, this), 300);
		},
		...
		deleteAllAction: function()
		{
			...
			let arrIds=Object.keys(this.items);
			for (let key in arrIds)
			{
				this.actionPool.deleteItem(arrIds[key]);
			}
		},
