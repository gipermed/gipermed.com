<?php
#####################################
define('LAST_MODIFIED','25.11.2019');
#####################################
...
if ($lang=='en')
{
	...
	$M['SELF_VERSION_DESC']='Can be downloaded from <a href="http://www.1c-bitrix.ru/download/scripts/bitrix_server_test.php">http://www.bitrix24.com</a>';
	$M['SELF_OLD']='Outdated';
	$M['SELF_NEW']='Fresh';
	...
	$M['SUPPORT_LINK']='<a 
	href="https://www.bitrix24.com">www.bitrix24.com</a>  |  <a 
	href="https://helpdesk.bitrix24.com/">Support</a>';
	...
	$M['PHP_VER_DESC']='Required version: '.$PHP_MIN;
	...
	<br>If there is no PHP accelerator, analysis of <a href="?phpinfo=Y">phpinfo()</a> is required';
	$M['CONFLICT']='conflict';
	...
	$M["max_input_vars"] = "Must be 10000 or higher";
}
else
{
	...
	$M['SUPPORT_LINK']='<a href="http://www.1c-bitrix.ru">www.1c-bitrix.ru</a>  |  <a href="http://www.1c-bitrix.ru/support/">Техподдержка</a>';
	...
	$M['PHP_VER_DESC']='Требуется '.$PHP_MIN;
	...
	$M['MCRYPT_TEST']='Модули шифрования';
	...
	$M['SYSUPDATE_DESC']='Осуществляется попытка подключиться к серверу www.1c-bitrix.ru на порт 80';
	...
	$M['MYSQL_REQ']='Минимальные требования: '.$MYSQL_MIN.' и выше. Альфа и бета версии не допускаются.';
...
// Check for updates
$strError = '';
if (!$_REQUEST['UPDATE_SUCCESS'] && !file_exists(dirname(__FILE__).'/bitrix_server_test.debug'))
{
...
// MySQL functions
debug(__LINE__);
$mysql = function_exists("mysql_connect") ? 'mysql' : (function_exists("mysqli_connect") ? 'mysqli' : 0);
...
{
	if ($mysqli = function_exists('mysqli_connect'))
	...
	if ($conn)
	{
		...
		$res=bx_mysql_query("SELECT version()");
		$f=bx_mysql_fetch_row($res);
		$pr=array(GM('MYSQL_VER'),GM('MYSQL_REQ'),1);
		show($pr,$f[0],version_compare($f[0], $MYSQL_MIN, '<'));

		$res=bx_mysql_query("SHOW VARIABLES LIKE 'sql_mode'");
		while($f=bx_mysql_fetch_row($res))
			show(array($f[0],GM('SQL_MODE_DESC'),1),"&nbsp;".$f[1],@preg_match("#strict#i",$f[1]));

		$res=bx_mysql_query("SHOW VARIABLES LIKE 'character\_set\_%'");
		while($f=bx_mysql_fetch_row($res))
...
debug(__LINE__);
				for($i=0;$i<1000;$i++)
				{
					if (!bx_mysql_query("INSERT INTO ".$name." VALUES ('test1','test2','test3','test4')"))
					{
						$good=false;
						break;
					}
				...
				if (bx_mysql_query("DROP TABLE ".$name))
					show($pr,1,0);
				else
					show($pr,0,1);
			} else
...
function bx_mysql_fetch_row($res)
{
	global $conn;
